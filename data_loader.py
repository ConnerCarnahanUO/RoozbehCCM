...
class RoozbehLabDataset:
    ...
    def load_spiking_data(file):
        ...
        for trial in range(n_trials):
            ...
            analog_i = [[]]
            for data_num in range(analog_val_num):
                analog_i.append(np.vstack([FIRA[2][trial,3].flatten()[data_num].flatten(),FIRA[2][trial,2].flatten()[data_num].flatten()]))
            analog_i.pop(0)
            analog.append(analog_i) #Analog stored as [trial][analog_var][i = 0 for time 1 for data,sample]

        def get_indices(event_string):
            ind = [i for i,x in enumerate(events) if x == event_string]
            return(ind[0])
         
        def analog_data_vs_time(dataframe):
            analog_data = [[]]
            analog_t = [[]]
        ...
        for trl_i in range(n_trials):
            ...
            for v_i in range(analog_val_num):
                analog[trl_i][v_i][0,:] += trl_offset
        ...
        data_dict['analog'] = analog #should at some point allow this to be indexed by the data type name

        return(data_dict)
    @staticmethod
    def ConcatenateTrialAnalogData(analoglist):
        return [np.hstack([analoglist['analog'][i][j] for i in range(len(analoglist['analog']))]) for j in range(len(analoglist['analog'][0])) ]
